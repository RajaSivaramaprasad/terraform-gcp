...
# or you can write "ansible playbooks"
  ...
  # Provisioner block "remote-exec" on the remote machine
 ...
 # provisioner block "local-exec" on the local machine
  provisioner "local-exec" {
    command = "echo 'Instance ${each.key} created with IP: ${self.network_interface[0].access_config[0].nat_ip}'"
    #command = each.key == "ansible" ? "echo 'Ansible vm created with IP: ${self.network_interface[0].access_config[0].nat_ip}'" : "echo 'Created vm: ${each.key}'"
  }
